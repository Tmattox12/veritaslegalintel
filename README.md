...
- **No case-specific hardcoding**: Removes all real party names (Parent A, Parent B, [Child], Parent B, [Child], [Child]), real dollar figures, real document paths, and real account numbers.
...
```bash
grep -r "Template Matter\|Parent A\|Parent B\|[Child]\|Parent B\|[Child]\|[Child]" webapp-template/
```
